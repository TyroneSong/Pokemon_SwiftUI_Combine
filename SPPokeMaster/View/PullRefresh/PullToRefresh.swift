...
extension PullToRefreshView: View where Header: View, Content: View, Footer: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            header
                ...
                .anchorPreference(key: HeaderBoundsPreferenceKey.self, value: .bounds) {
                    [.init(bounds: $0)]
                }
            content()
                .anchorPreference(key: FooterBoundsPreferenceKey.self, value: .bounds) {
                    [.init(bounds: $0)]
    ...
    var dynamicHeaderOpacity: Double {
        if headerRefreshData.refreshState == .invalid { return 0.0 }
        if headerRefreshData.refreshState == .stopped { return headerRefreshData.progress }
        return 1.0
    }
    
    var dynamicFooterOpacity: Double {
        if footerRefreshData.refreshState == .invalid { return 0.0 }
        if footerRefreshData.refreshState == .stopped { return footerRefreshData.progress }
        return 1.0
    }
